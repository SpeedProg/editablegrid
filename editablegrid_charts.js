...
EditableGrid.prototype.renderBarChart = function(divId, title, labelColumnIndexOrName, options)
{
  ...
  chart.xAxis = {
      title: { text:  this.legend || this.getColumnLabel(labelColumnIndexOrName) },
      labels: { rotation: this.rotateXLabels }
  };

  // one category for each row
  chart.xAxis.categories = [];
  for (var r = 0; r < rowCount; r++) {
    if (this.skipRow(r)) continue;
    var label = this.getRowAttribute(r, "barlabel"); // if there is a barlabel attribute, use it and ignore labelColumn
    chart.xAxis.categories.push(label ? label : this.getValueAt(r, cLabel));
  }
  ...
  for (var c = 0; c < columnCount; c++) {
    ...
    var serie = {
        name: this.getColumnLabel(c),
        stack: this.getColumnStack(c),
        yAxis: yAxisIndex,
  ...
  if (cValue == cLabel) {
    ...
    for (var r = 0; r < rowCount; r++) {
      ...
      if (value !== null && !isNaN(value)) serie.data.push({
        y : value,
        name: (label ? label : this.getValueAt(r,cLabel)),
